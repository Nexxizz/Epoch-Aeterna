"""Validate and render the saved swordsman: Blender --background blender/unit_swordsman.blend --python tools/blender/inspect_swordsman.py."""
import os
import sys
import math
import json
...
out=os.path.join(preview.lib_scene.REPO_ROOT,'docs/previews/swordsman')
...
for action in bpy.data.actions:
    ...
    assert grip_error<.005,(action.name,'equipment leaves hand',grip_error)
...
if '--check-only' not in sys.argv:
    ...
    for clip,frame,view in [('Idle',0,'front'),('Idle',0,'side'),('Run',6,'three-quarter'),
        ('Attack',13,'side'),('Attack',21,'three-quarter'),('Death',72,'three-quarter')]:
